...
public function superAssets(): string
{
    ...
    // الإرجاعات - Query item_order directly for items with return status
    ...
    $stats = [
        ...
        'total_assets' => count(array_unique(array_column($operations, 'asset_number'))),
    ];
...
public function assetCycle($assetNum = null): string
{
    ...
    $usersModel = new \App\Models\UserModel(); // ✅ Fixed: Changed from UsersModel to UserModel
    ...
    // Handle return status based on usage_status_id
    if (in_array($assetInfo->usage_status_id, [2, 4, 5])) {
        ...
        $displayStatus = match ((int)$assetInfo->usage_status_id) {
            2 => 'قيد الانتظار',
            4 => 'مقبول',
            5 => 'مرفوض',
            default => 'غير محدد'
        ...
        // Determine who returned the item - check created_by in both tables
        $returnedByName = 'غير محدد';
        $returnedByDept = 'غير محدد';
        $returnedByExt = '-';

        if ($lastTransfer) {
            // If there was a transfer, the person who received it (to_user) is returning it
            $returnedByName = $lastTransfer->user_name ?? 'غير محدد';
            $returnedByDept = $lastTransfer->user_dept ?? 'غير محدد';
            $returnedByExt = $lastTransfer->user_ext ?? '-';
        } else {
            // No transfer, check created_by - could be employee or user
            if ($assetInfo->created_by) {
                // First try employee table
                $employeeInfo = $employeeModel
                    ->where('emp_id', $assetInfo->created_by)
                    ->first();
                
                if ($employeeInfo) {
                    $returnedByName = $employeeInfo->name;
                    $returnedByDept = $employeeInfo->emp_dept;
                    $returnedByExt = $employeeInfo->emp_ext;
                } else {
                    // Try users table
                    $userInfo = $usersModel
                        ->where('user_id', $assetInfo->created_by)
                        ->first();
                    
                    if ($userInfo) {
                        $returnedByName = $userInfo->name;
                        $returnedByDept = $userInfo->user_dept;
                        $returnedByExt = $userInfo->user_ext;
                    }
                }
            }
        }
        ...
        $timeline[] = [
            ...
            'returned_by_dept' => $returnedByDept,
            'returned_by_ext' => $returnedByExt,
            'status' => $displayStatus,
            'usage_status_id' => $assetInfo->usage_status_id
        ];
    }
